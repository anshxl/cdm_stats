...
from cdm_stats.charts.heatmap import chart_map_strength, chart_elo_trajectory
...
def test_chart_map_strength_creates_file(db, tmp_path):
    dvs = db.execute("SELECT team_id FROM teams WHERE abbreviation = 'DVS'").fetchone()[0]
    output_path = tmp_path / "strength_DVS.png"
    chart_map_strength(db, dvs, str(output_path))
